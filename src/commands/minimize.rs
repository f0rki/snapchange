...
use crate::parexec::{gather_coverage_breakpoints, ParallelVMPool, SharedVMData};
...
pub(crate) fn run<FUZZER: Fuzzer>(
    ...
    */
    ...
    let min_params = &minparams;

    let cores = 32_usize;

    let vm_pool = ParallelVMPool::<FUZZER>::with_setup(
        project_state.clone(),
        std::num::NonZeroUsize::new(cores).unwrap(),
        |project_state, clean_snapshot, symbols, reset_breakpoints, coverage_breakpoints| {
            use MinimizeCodeCovLevel::{BasicBlock, Hitcounts};
            if matches!(min_params.codecov_level, Hitcounts | BasicBlock)
                && !project_state.coverage_basic_blocks.is_some()
            {
                anyhow::bail!("code coverage level requires breakpoint addresses!");
            } else if !matches!(min_params.codecov_level, MinimizeCodeCovLevel::None)
                && (symbols.as_ref().is_none() || symbols.as_ref().is_some_and(|s| s.is_empty()))
            {
                anyhow::bail!("code coverage level requires symbols!");
            }
            let have_bbs = project_state
            ...
            if have_bbs {
                    ...
                    || min_params.codecov_level == MinimizeCodeCovLevel::BasicBlock
                {
                    ...
                    // let symbols = symbols.context("code coverage level requires symbols")?;
                    let curr_clean_snapshot = clean_snapshot.read().unwrap();
                    let cr3 = Cr3(project_state.vbcpu.cr3);
                    for sym in symbols.unwrap().iter() {
                        if sym.address == 0 {
                            log::warn!("symbol at null {:?}", sym);
                        } else {
                            if let Ok(orig_byte) =
                                curr_clean_snapshot.read_byte(sym.address.into(), cr3)
                            {
                                coverage_breakpoints.insert(sym.address.into(), orig_byte);
                            }
                        }
                    }
                }
    ...
    let max_iterations = args.iterations_per_stage;
    ...
    let _res = filepaths
        ...
        // .try_for_each_with(vm_pool, |mut vm_pool, (input_fuzzcase, output_fuzzcase)| -> Result<()> {
        .try_for_each(|(input_fuzzcase, output_fuzzcase)| -> Result<()> {
            log::debug!("getting VM");
            let mut vm_pool = vm_pool.clone();
            let SharedVMData {
                ref mut fuzzer,
                ref mut fuzzvm,
                ..
                } = &mut *vm_pool.get_vm()?;
            // let mut fuzzer = &mut shared_vmdata.fuzzer;
            // let mut fuzzvm = &mut shared_vmdata.fuzzvm;
            // let coverage_breakpoints = &shared_vmdata.coverage_breakpoints;
            // let coverage_breakpoints = &fuzzvm.coverage_breakpoints;
            ...
            let cov_bps: Vec<VirtAddr> = fuzzvm.coverage_breakpoints.as_ref().map_or(vec![], |x| x.keys().cloned().collect());
            ...
            if last_execution.is_crash() {
                ...
                fuzzer.handle_crash(&input, fuzzvm, &output_fuzzcase)?;
            }
